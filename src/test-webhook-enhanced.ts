/**
 * Расширенный тест webhook системы
 */
...
async function testWebhookEnhanced() {
  console.log('🧪 Testing Enhanced Webhook System...\n');
...
async function testMultiPlatformSupport() {
  ...
  const manager = WebhookManager.getInstance();
  const telegramAdapter = new MockWebhookAdapter('telegram');
  const maxAdapter = new MockWebhookAdapter('max');

  // Очищаем предыдущие webhook'и
  await manager.dispose();

  // Регистрируем webhook'и для разных платформ
  manager.registerWebhook('telegram-1', telegramAdapter as any, {
    path: '/webhook/telegram/1'
  });
  manager.registerWebhook('max-1', maxAdapter as any, {
    path: '/webhook/max/1'
  });
  ...
  const platforms = registered.map(w => w.platform);
  console.log(`    Platforms: ${platforms.join(', ')}`);

  if (registered.length !== 2) {
    throw new Error(`Expected 2 webhooks, got ${registered.length}`);
  }

  if (!platforms.includes('telegram') || !platforms.includes('max')) {
    throw new Error('Should have both telegram and max platforms');
  }
  ...
  if (overallStats.totalWebhooks !== 2) {
    throw new Error(`Expected 2 total webhooks, got ${overallStats.totalWebhooks}`);
  }
}
...
if (require.main === module) {
  testWebhookEnhanced()
    .then(results => {
      if (results.failed === 0) {
        console.log('\n🎉 All enhanced webhook tests passed!');
        process.exit(0);
      } else {
        console.log('\n💥 Some enhanced webhook tests failed!');
        process.exit(1);
      }
    })
    .catch(error => {
      console.error('\n💥 Enhanced webhook test suite failed:', error);
      process.exit(1);
    });
}

export { testWebhookEnhanced };